...
def msg_should_have_func_def(func_name):
    return "Programmis peaks olema defineeritud funktsioon '{func}'. Ei leidnud selle funktsiooni definitsiooni.".format(func=func_name)

def msg_should_not_have_more_than_one_def(func_name):
    return "Leidsin programmist mitu funktsiooni '{func}' definitsiooni. Funktsioone tuleb defineerida vaid üks kord!".format(func=func_name)
...
def msg_should_have_returned_none(func_name, args_repr, ret_repr):
    return "Andsin funktsioonile '{func_name}' argumendiks {args_repr} ja ootasin, et see ei ' \
           'tagastaks mitte midagi. Kuid funktsioon tagastas {ret_repr}".format(func_name=func_name, args_repr=args_repr, ret_repr=ret_repr)
...
def msg_wrong_return_value(func_name, args_repr, exp_ret, act_ret):
    ...
    return "Andsin funktsioonile '{func_name}' argumendiks {args_repr} ja ootasin, et see tagastaks " \
           "{exp_ret} kuid see tagastas {act_ret}".format(func_name=func_name, args_repr=args_repr, exp_ret=exp_ret, act_ret=act_ret)
...
def msg_wrong_number_of_params(func_name, exp_no_of_params, act_no_of_params):
    return "Funktsioon '{func}' peaks olema defineeritud {exp} parameetriga, kuid praegu " \
           "leidsin definitsioonist {act} parameetrit.".format(func=func_name, exp=exp_no_of_params, act=act_no_of_params)

def msg_inner_function_call_missing(outer_func, inner_func):
    return "Ei leidnud funktsiooni '{outer_func}' seest funktsiooni '{inner_func}' väljakutset.".format(outer_func=outer_func, inner_func=inner_func)

def msg_recursive_function_call_missing(func_name):
    return "Ei leidnud funktsiooni '{func_name}' seest rekursiivset pöördumist enda ('{func_name}') poole.".format(func_name=func_name)

def msg_missing_expected_string_from_out(exp_out, actual_out):
    return "Ei leidnud programmi väljundist oodatud vastust {exp_out} Programmi väljund oli {actual_out}".format(exp_out=exp_out, actual_out=actual_out)

def msg_found_unexpected_string_from_out(unexp_out, actual_out):
    return "Leidsin programmi väljundist ootamatu vastuse {unexp} Programmi väljund oli {actual}".format(unexp=unexp_out, actual=actual_out)

def msg_no_distinct_return_values_from_random(func_name, samples):
    # TODO: make tööle n > 0 argumendiga
    return "Rakendasin funktsiooni '{func_name}' {n} korda ja iga kord tagastati sama tulemus.".format(func_name=func_name, n=samples)

def msg_illegal_return_value_from_random(func_name, actual_ret):
    # TODO: make tööle n > 0 argumendiga
    return "Funktsioon '{func_name}' tagastas ebakorrektse väärtuse: {act_ret}".format(func_name=func_name, act_ret=actual_ret)
...
def must_have_equal_return_values(expected_func_object, actual_func_object, function_name, *func_args,
    ...
    if expected_return is None:
        ...
        assert equalizer(actual_return, expected_return), \
            msg_wrong_return_value(function_name, args_repr, ret_representer(expected_return), ret_representer(actual_return))
...
def must_have_correct_output_str(stdin, stdout, inputs, expected_strings, unexpected_strings,
    ...
    for exp in expected_strings:
        if ignore_case:
            assert exp.upper() in out.upper(), msg_missing_expected_string_from_out(exp_out_representer(exp), act_out_representer(out))
        else:
    ...
    for unexp in unexpected_strings:
        if ignore_case:
            assert unexp.upper() not in out.upper(), msg_found_unexpected_string_from_out(exp_out_representer(unexp), act_out_representer(out))
        else:
            assert unexp not in out, msg_found_unexpected_string_from_out(exp_out_representer(unexp), act_out_representer(out))
